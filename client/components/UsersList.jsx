	...
	render() {
		return (
			<div className="UsersList">
						...
						<tbody>
							...
							{this.props.usersList.map((element, index) => (
								<tr key={index}>
									...
									<td>
										{' '}
										<span onClick={e => this.props.onDeleteUser(element._id)}>
											&#10007;
										</span>
									</td>
								</tr>
				...
				<p>
					Role:{' '}
					<select onChange={e => this.props.onChangeRole(e.target.value)}>
						<option value="developer">developer</option>
						<option value="manager">manager</option>
					</select>
				</p>

				<button onClick={e => this.props.onAddUser(this.props.user)}>
					Add developer
				</button>
			</div>
		);
