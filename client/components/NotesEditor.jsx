			...
			<div className="MenuContainer">
				...
				<div className="NotesEditor">
					...
					<select
						...
						onClick={e => this.props.onChangeNoteProject(e.target.value)}
					>
						{this.props.projectList.length > 0 ? (
							this.props.projectList.map((elem, index) => {
								return (
									<option key={index} value={elem.title}>
										{elem.title}
									</option>
								);
							})
					...
					<select
						onClick={e => this.props.onChangeNoteDeveloper(e.target.value)}
					>
						{this.props.usersList.length > 0 ? (
							this.props.usersList.map((elem, index) => {
								return (
									<option key={index} value={elem.name + ' ' + elem.lastName}>
					...
					<select
						...
						onClick={e => this.props.onChangeNoteStatus(e.target.value)}
					>
					...
					/>
					<button onClick={this.addNote}>Create task</button>
				</div>
				<UsersList />
			</div>
		);
