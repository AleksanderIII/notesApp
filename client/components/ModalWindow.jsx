...
class ModalWindow extends React.Component {
	...
	render() {
		return this.props.modalDate.color ? (
			<div
			...
			>
				<div
					className="Modal"
					style={{ background: this.props.modalDate.color }}
				>
					<h3>Project: {this.props.modalDate.project}</h3>
					<h3>Task name: {this.props.modalDate.title}</h3>
					<div className="Modal_edit">
						<p> Comments: </p>
						{this.props.modalDate.text.map((elem, index) => (
							<p key={index} className="Modal_edit_comment">
								{elem}
								<span
									id={index}
						...
						{this.props.modal.showInputCommentEditor && (
							<p>
								Change comment:{' '}
								{this.props.modal.text[this.props.modal.editableComment]}
								<input
									type="text"
								...
								/>
								<span onClick={e => this.props.onCommentEdited()}>
									&#10004;
								</span>
							</p>
						)}
						...
						<span
							onClick={e =>
								this.props.onAddComment(this.props.modal.newComment)
							}
						>
							&#10004;
						</span>
					...
					{this.props.modal.showStatusSelection && (
						<p>
							Change status to:
					...
					<p>
						Responsible for implementation: {this.props.modalDate.developer}
					</p>
					<p className="Modal_date">
						{' '}
						Creation date:{' '}
						{new Date(this.props.modalDate.date).toLocaleString(
							'en-us',
							options
						)}
					</p>
					<span
						...
						onClick={e => this.props.onPostNoteChanges(this.props.modal)}
					>
						&#10004;
					</span>
				</div>
