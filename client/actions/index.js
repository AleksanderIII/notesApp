...
export const deleteUser = id => dispatch => {
	...
	fetch(`http://localhost:8080/users/${id}`, fetchOptions)
		...
		.then(data => {
			dispatch({ type: SET_INITIAL_DEVELOPER, payload: data });
		});
};
...
export const postNoteChanges = note => dispatch => {
	...
	fetch(`http://localhost:8080/notes/${note._id}`, fetchOptions)
		.then(response => response.json())
		.then(data => {
			dispatch({ type: POST_NOTE_CHANGES });
			return data;
		})
		.then(data => {
...
export const getProjectsList = () => dispatch => {
	fetch('http://localhost:8080/projects')
		.then(response => response.json())
		.then(data => {
			dispatch({ type: GET_PROJECTS_LIST, payload: data });
			return data;
		})
		.then(data => {
			dispatch({ type: SET_INITIAL_PROJECT, payload: data });
		});
};
